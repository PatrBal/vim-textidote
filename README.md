...
## Features
 ...
 - A scratch buffer shows up, listing all the errors.  Navigate the errors using `[[` and `]]`, hit `<Enter>` on an error in the error scratch buffer to jump to that error and, then, hit `<Tab>` to fix it.
