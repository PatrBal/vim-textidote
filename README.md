...
- Another alternative is to use [YaLafi][YaLafi] (Yet another LaTeX filter) together with [vim-LanguageTool][vim-LanguageTool], [vim-grammarous][vim-grammarous], or... `vim-textidote`. This should provide an experience somewhat similar to `vim-textidote` with `TeXtidote`, but handling custom dictionaries in `LanguageTool`, and therefore in `YaLafi`, is not very convenient.
