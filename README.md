...
The plugin [vim-LanguageTool][vim-LanguageTool] is a full-featured interface of LanguageTool with Vim.  This plugin offers the same clean interface based on TeXtidote instead of LanguageTool. As a consequence, all the errors based on LaTeX or Markdown markup are skipped.  In addition, the plugin is now able to make asynchronous calls to TeXtidote, both in Neovim and regular Vim, so that Vim (or Neovim) does not get frozen during the TeXtidote analysis (which can take several minutes for large files).  As a consequence, the plugin requires at least Vim 8.
